...
public class Handler : IConsumer<FindOrderRequest>
{
    ...
    public async Task Consume(ConsumeContext<FindOrderRequest> context)
    {
        ...
        await context.RespondAsync(new OrderDto(order!.Id, order.CustomerId, order.TotalItems, order.OrderItems.Select(oi => new OrderItemDto(oi.ProductId, oi.Quantity))));
    }
}