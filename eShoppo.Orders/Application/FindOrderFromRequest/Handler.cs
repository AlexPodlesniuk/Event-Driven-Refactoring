...
internal class Handler : IConsumer<FindOrderRequest>
{
    ...
    public async Task Consume(ConsumeContext<FindOrderRequest> context)
    {
        ...
        await context.RespondAsync(new FindOrderResponse(order!.Id, order.OrderNumber, order.CustomerId, order.TotalItems, order.OrderItems.Select(oi => new OrderItemDto(oi.Item.ProductId, oi.Item.Quantity))));
    }
}